<?
	...
	if($search) {
		$sqlPlus = " AND  nama_perusahaan LIKE'%$search%'";
		
	}

	$sql = "SELECT * FROM mt_perusahaan WHERE flag_mitra=2 $sqlPlus ORDER BY nama_perusahaan";
	...
	<?
			...
			while ($tampil=$rsPaging->FetchRow()) {
				...
				$tampil["action_hapus"]="<a href='#' title='Hapus Data' onclick='hapus_perusahaan($id_perusahaan)'>
							<i class='las la-trash-alt icon-lg text-danger '></i>
							</a>";
				$tampil["action_edit"]="<a href='#' title='Edit  Data' onclick='ubah_perusahaan($id_perusahaan)'>
							<i class='las la-edit icon-lg text-success '></i>
							</a>";
				
				$tampil["perjanjian"]="<button class='btn btn-primary'>Perjanjian</button>";
							
				$status = $flag_status;
				$tampil["telpon1"]		= $telpon;
