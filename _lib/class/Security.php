...
if (!class_exists("Security")) {
	...
	class Security extends AvObjects {
		...
		public function __construct($db,$userName="",$userPass="",$bypass="") {
			parent::__construct();
			$this->_db = $db;
			$this->_validUser=false;

			if (($userName!="" && $userPass!="") || ($userName!="" && $bypass!="")) {
				//die("disini");
				$this->_cekUser($userName,$userPass,$bypass);
			}
		...
		private function _cekUser($userName="",$userPass="",$bypass="") {
			...
			while ($check_user=$result_id->FetchRow()) {
				...
				$sql_tutup_sblmnya="UPDATE log_user_login SET logoff_time='".date("Y-m-d H:i:s")."' WHERE logoff_time IS NULL AND session_id='".session_id()."'";
